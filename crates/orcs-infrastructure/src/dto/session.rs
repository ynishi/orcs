...
/// V4.1.0: Fix participant_models to avoid TOML null serialization errors
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize, Versioned)]
#[versioned(version = "4.1.0")]
pub struct SessionV4_1_0 {
    /// Unique session identifier
    pub id: String,
    /// Human-readable session title
    pub title: String,
    ...
    pub participant_backends: HashMap<String, String>,
    /// Participant persona ID to model name mapping (e.g., "claude-sonnet-4-5-20250929")
    /// Changed from HashMap<String, Option<String>> to avoid TOML null serialization errors
    #[serde(default)]
...
impl From<ContextMode> for ContextModeDto {
    fn from(mode: ContextMode) -> Self {
        match mode {
            ContextMode::Rich => ContextModeDto::Rich,
            ContextMode::Clean => ContextModeDto::Clean,
        }
    }
}

impl From<ContextModeDto> for ContextMode {
    fn from(dto: ContextModeDto) -> Self {
        match dto {
            ContextModeDto::Rich => ContextMode::Rich,
            ContextModeDto::Clean => ContextMode::Clean,
        }
    }
}
...
/// Represents V4.4.0 of the session data schema.
/// Added sandbox_state for git worktree-based isolated development.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize, Versioned)]
#[versioned(version = "4.4.0")]
pub struct SessionV4_4_0 {
    /// Unique session identifier
    pub id: String,
    /// Human-readable session title
    pub title: String,
...
impl version_migrate::FromDomain<Session> for SessionV4_3_0 {
    fn from_domain(session: Session) -> Self {
        ...
        SessionV4_3_0 {
            ...
            participant_colors,
            participant_backends,
            // Filter out None values from participant_models to avoid TOML null serialization errors
...
/// Convert domain model to SessionV4_4_0 DTO for persistence.
impl version_migrate::FromDomain<Session> for SessionV4_4_0 {
    fn from_domain(session: Session) -> Self {
        let Session {
            id,
            title,
        ...
        // Convert HashMap<String, Option<String>> to HashMap<String, String>
        let participant_models = participant_models
            .into_iter()
            .filter_map(|(k, v)| v.map(|val| (k, val)))
            .collect();
